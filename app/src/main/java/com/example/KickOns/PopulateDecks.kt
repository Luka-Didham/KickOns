 ...
 *
 * @see com.example.KickOns.DeckDAO
...
class PopulateDecks(ctx: Context) {
  ...
  */
  var aList = listOf(
    ...
    arrayOf("Court Fines: @player1 You have 30 seconds to clear your name for why you don't deserve a maximum penalty. After your plea all other players put their thumbs down for drinks or up for forgiveness" ,3),
    arrayOf("@player1 every time you say \"jeep\" take a penalty", 3),
    arrayOf("@player1, and @player2. Where are the weirdest places you've had sex. Weirdo take a penalty", 0),
    ...
    arrayOf("@player1, choose the next song. Any player who doesn't like it take 5 penalties", 0),
    ...
    arrayOf("@player1 if you have ever ended up in a serious relationship with a fuckbuddy give out 3 penalties or drink them if not", 0),
    ...
    arrayOf("Everyone must snarl everytime they take a sip for the next 3 cards. Fail to do so take a max penalty", 2),
    arrayOf("@player1 you are the pig. You can only communicate with pig oinks for the next 3 cards", 0),
    arrayOf("Buffalo", 2),
    arrayOf("Kiss party: go around circle and kiss everyone to the left and right or take 3 penalties. @player1 you start", 0),
    arrayOf("Gentleman Rules: You must address all other players by Sir or Miss or finish your drink for the next 3 cards", 2),
    ...
    arrayOf("Medusa: @player1 if any other player looks you in the eyes for the next 3 cards they must finish their drink", 1),
  ...
  /**
   * Iterates through all cards in an array and adds them to a database
   *
   * @returns A full deck built from a hard-coded array
   */
  fun insert(){
    cardList.clear()
    for (t in tList){
  ...
  /**
   * Adds a given line, and card type (value 0-3) to a deck.
   *
   * @param type The value 0-3 of the card type
   * @param text The text of the card being added
   * @returns Each individual card to the deck
   */
  private fun addCards(type: Int, text: String){
